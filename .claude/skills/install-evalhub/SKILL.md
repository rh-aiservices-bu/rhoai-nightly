...
Every unexpected symptom: capture in `$LOGDIR`, record it: product bug → `docs/issues/` (+ `docs/known-issues.md` if user-facing); carried fix → `docs/workarounds.md`, prefer a committed repo fix over hand-patching the cluster, then re-run. Do not silently swallow errors.
...
Summarize: cluster URL + RHOAI CSV; EvalHub/MLflow/DSPA Ready states; pod counts in both namespaces; hook Job status; DSPA routes; Application sync/health; any problems (with `docs/issues/`/`docs/workarounds.md` pointers + repo-side fix commits); log directory path.
