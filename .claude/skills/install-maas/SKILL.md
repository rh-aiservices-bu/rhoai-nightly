...
2. Record it: product bug → `docs/issues/` (+ `docs/known-issues.md` if user-facing); carried fix → `docs/workarounds.md` with symptom / detection / root cause / remove-when
...
- **Problems encountered** (with pointers to `docs/issues/`/`docs/workarounds.md` entries and any repo-side commits that landed fixes)
- Log directory path
