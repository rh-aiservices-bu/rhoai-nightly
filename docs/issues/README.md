...
| [dsc-maas-deprecated-field-ignored.md](dsc-maas-deprecated-field-ignored.md) | **NOT FILED** — target RHOAIENG (opendatahub-operator AI Gateway module); draft in file | Open — found fresh n56g8 2026-09-25 on 3.6.0-ea.2: `kserve.modelsAsService: Managed` ignored because the CRD defaults `aigateway.modelsAsAService` to `Removed`, defeating the operator's `== ""` fallback. Repo migrated to the new field (not a workaround). **3.5→3.6 upgrade impact (MaaS torn down) likely but unverified — confirm before filing** |
| [gitops-catalog-ose-cli-latest.md](gitops-catalog-ose-cli-latest.md) | **NOT FILED** — target GitHub issue on [redhat-cop/gitops-catalog](https://github.com/redhat-cop/gitops-catalog) (searched 2026-09-25: nothing upstream). Not an RHOAI bug | Open — found fresh n56g8 2026-09-25: `job-gpu-console-plugin` ImagePullBackOff (registry.redhat.io refuses `ose-cli:latest`), `nvidia-operator` app stuck Progressing, NVIDIA console plugin never enabled. Cosmetic; no workaround |
...
tab-keying bug; added 2026-08-07), gitops-catalog ose-cli `:latest` (GitHub
issue on redhat-cop/gitops-catalog; added 2026-09-25), 3.6 deprecated
`kserve.modelsAsService` ignored (RHOAIENG; added 2026-09-25) — plus the prepared
